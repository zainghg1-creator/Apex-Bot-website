...
console.log('✅ NEUE dashboard.js mit Embed-Vorschau und Optionen-Tab!');
...
function addTicketOption(data = null) {
  ...
  // Diese Funktion bleibt nur für Kompatibilität mit bestehendem Code.
}
function collectTicketOptions() {
  return []; // Wird nicht mehr verwendet
}
...
function applyTicketConfig(cfg) {
  // Wird nur noch für die globalen Felder verwendet (Panel-Kanal etc.)
  ...
  try {
    switch (moduleName) {
      case 'welcome':
        ...
        break;
      case 'tickets':
        // Hier speichern wir jetzt nur noch die globalen Felder (Panel-Kanal etc.)
        // Die eigentlichen Panel-Daten werden über saveEditView gespeichert.
        payload = {
...
// ============================================================
// TICKET ÜBERSICHT & BEARBEITUNGSANSICHT MIT EMBED-VORSCHAU + OPTIONEN
// ============================================================
...
// ------ Embed-Vorschau aktualisieren (in der Bearbeitungsansicht) ------
function updateEditPreview() {
  ...
  if (!preview) return;
  preview.querySelector('.discord-embed-title').textContent = title;
  preview.querySelector('.discord-embed-desc').textContent = desc;
  preview.style.borderLeftColor = color;
  const img = preview.querySelector('.discord-embed-image');
  if (image) {
    img.src = image;
...
// ------ Bearbeitungsansicht mit Embed-Vorschau und Optionen ------
async function showEditView(index) {
  ...
  if (index !== null && options[index]) {
    ...
    data = {
      ...
      claimEnabled: false,
      buttons: [],
      options: [] // die verknüpften Kategorien (Dropdown-Optionen)
    };
  }

  // --- HTML mit Embed-Vorschau, Panel-Dropdown und Tabs ---
  let html = `
    <div class="card form-card" style="max-width:100%;">

      <!-- Panel-Auswahl (Dropdown) -->
      <div style="display:flex; gap:16px; align-items:center; margin-bottom:16px; flex-wrap:wrap;">
        <label style="font-weight:600; color:var(--text-muted);">Panel wechseln:</label>
        <select id="edit-panel-select" style="flex:1; background:var(--bg-base); border:1px solid var(--border-subtle); border-radius:10px; padding:10px 14px; color:var(--text-primary);">
        ...
        <button class="btn btn-secondary" onclick="switchToSelectedPanel()" style="flex-shrink:0;">Wechseln</button>
      </div>
      ...
      <div style="margin-bottom:20px; background:var(--bg-surface); border-radius:12px; padding:16px; border:1px solid var(--border-subtle);">
        <label style="font-weight:600; color:var(--text-muted); display:block; margin-bottom:10px;">📺 Live-Vorschau</label>
        <div class="discord-embed" id="edit-embed-preview" style="border-left-color:${data.color || '#ffffff'};">
          <div class="discord-embed-body">
            ...
            <img class="discord-embed-image ${data.image ? '' : 'hidden'}" src="${data.image || ''}" style="max-height:160px; width:100%; border-radius:6px; margin-top:10px; object-fit:cover;">
            <div class="discord-embed-footer">
              <img src="apex_logo.png" style="width:18px;height:18px;border-radius:50%;">
      ...
      <div style="display:flex; gap:4px; border-bottom:1px solid var(--border-subtle); margin-bottom:1.5rem; flex-wrap:wrap;">
        ...
        <button class="edit-tab-btn" data-edit-tab="options" onclick="switchEditTab('options')" style="background:transparent;border:none;color:var(--text-muted);padding:10px 16px;font-weight:600;cursor:pointer;border-bottom:2px solid transparent;transition:all 0.2s;font-family:inherit;font-size:0.85rem;">Optionen</button>
      </div>
      ...
      <div id="edit-tab-general" class="edit-tab-content">
        ...
        <div class="form-group">
          ...
          <input type="text" id="edit-panel-name" value="${escapeHtml(data.panelName || '')}" placeholder="Support" oninput="updateEditPreview();">
        </div>
        <div class="form-group">
      ...
      <div id="edit-tab-options" class="edit-tab-content hidden">
        <div class="form-group">
          <label style="font-weight:700;">Dropdown-Optionen (Kategorien)</label>
...
window.switchToSelectedPanel = async function() {
  ...
  if (val === 'new') {
    // Neues Panel – leere Bearbeitungsansicht
    editingIndex = null;
    showEditView(null);
  } else {
...
window.saveEditView = async function() {
  ...
  const options = collectOptions().filter(o => o.label.trim()); // die verlinkten Kategorien
  ...
  try {
    ...
    const newData = {
      ...
      claimEnabled,
      buttons,
      options // <-- die verlinkten Kategorien
    };
