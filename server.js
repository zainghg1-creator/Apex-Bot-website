...
const { createSupabaseClient, makeGuildConfigModel, makeButtonActionModel } = require('./supabaseCompat');
...
// SUPABASE VERBINDUNG
// ============================================================
// Supabase/PostgREST ist zustandslos über HTTP - es gibt kein "Verbinden"
// wie bei Mongoose. connectToDatabase() bleibt als No-Op-Funktion erhalten,
// damit die vielen bestehenden `await connectToDatabase();`-Aufrufe im Code
// unten nicht einzeln entfernt werden müssen.
...
if (!supabaseClient) {
  console.log('⚠️ Keine SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY - laufe ohne DB');
}
...
if (supabaseClient) {
  GuildConfig = makeGuildConfigModel(supabaseClient);
  ButtonAction = makeButtonActionModel(supabaseClient);
  // Kein manuelles TTL-Handling nötig - erledigt der pg_cron-Job aus
  // supabase_schema.sql (löscht button_actions älter als 1h).
}
