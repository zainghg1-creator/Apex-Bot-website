...
from supabase_compat import ReturnDocument, DBPing, build_collections
import logging
...
# ============ SUPABASE ============
# Ersetzt die frühere MongoDB-Verbindung. supabase_compat.py stellt dieselbe
# find_one/find/insert_one/update_one/delete_one/find_one_and_update-API wie
# pymongo bereit, damit der Rest von main.py unverändert bleiben kann.
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_KEY = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
_COLLECTION_VARS = [
    "guild_configs", "giveaways_collection", "team_warns_collection",
    "counting_collection", "levels_collection", "button_actions",
    "applications_collection", "minigame_rounds_collection",
    "transcripts_collection", "shifts_collection", "shift_stats_collection",
    "quiz_stats_collection", "logouts_collection", "tickets_collection",
]
if SUPABASE_URL and SUPABASE_KEY:
    try:
        supabase_client = create_client(SUPABASE_URL, SUPABASE_KEY)
        db = DBPing(supabase_client)
        _collections = build_collections(supabase_client)
        for _var in _COLLECTION_VARS:
            globals()[_var] = _collections[_var]
        logger.info("✅ Supabase erfolgreich verbunden")
    except Exception as e:
        logger.error(f"❌ Supabase Verbindungsfehler: {e}")
        db = None
        for _var in _COLLECTION_VARS:
            globals()[_var] = None
else:
    logger.warning("⚠️ Keine SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY gefunden – laufe ohne DB")
    db = None
    for _var in _COLLECTION_VARS:
        globals()[_var] = None
...
# Supabase/Postgres hat kein 1:1-Äquivalent zu MongoDB Change Streams.
# Statt eines Live-Watchers verlassen wir uns auf den bereits vorhandenen
# TTL-Cache (_CONFIG_CACHE_TTL, aktuell 8s) weiter oben: Dashboard-Änderungen
# sind dadurch spätestens nach 8 Sekunden im Bot sichtbar - ausreichend für
# Config-Änderungen, die nicht sekundengenau ankommen müssen. Falls doch mal
# Echtzeit gebraucht wird, kann das später über Supabase Realtime
# (postgres_changes) nachgerüstet werden.
async def guild_config_watch_loop():
    logger.info(
        "[CACHE] Live-Change-Stream entfällt mit Supabase - Konfigurationsänderungen "
        f"werden stattdessen über den bestehenden {_CONFIG_CACHE_TTL}s-TTL-Cache übernommen."
    )
    return
